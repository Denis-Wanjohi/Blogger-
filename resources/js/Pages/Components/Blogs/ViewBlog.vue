            ...
            >
              ...
              <div class="max-h-[85%]  overflow-y-scroll" style="scrollbar-width:none">
                <div class=" mb-3 overflow-y-scroll pl-2" style="scrollbar-width:none">
                  <p><span class="pl-5"></span>{{ post.data.likes[0].user_id}} </p>
                  <!-- <p><span class="pl-5"></span>{{likes.data[post.data.id -  1 ].likes}} </p> -->
                  <!-- <p>{{ $page.props.auth.user.id }} </p> -->

                  <p  v-if="post.data.likes[0].user_id !== auth ">{{ $page.props.auth.user.id }}</p>
                  <p v-if="post.data.likes[0].blog_id === post.data.id"  >two</p>
                  <p  v-if="post.data.likes[0].liked === 1" >{{ post.data.likes}}</p>
                    {{ page }}

                  <!-- check if the auth USER has LIKED this BLOG -->
                  <hr>
                  <!-- user -->
                  <p>user id {{ $page.props.auth.user.id }}</p>
                  <!-- has liked? -->
                  <div v-for="likes in post.data.likes">
                    <p v-if="likes.user_id === $page.props.auth.user.id">{{likes.liked}} liked this post</p>
                  ...
                  {{ isliked }}
                  <hr>
                  <!-- show that the auth has liked this -->
            ...
            <div class="w-full h-[40px]  flex justify-between">
              <div class="flex">
                <div id="like" class="place-items-center flex">
                  <span class="pl-2 font-semibold">{{ likes.data[post.data.id -  1 ].likes }}</span>
